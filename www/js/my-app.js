...
//---------------------------------------------------iconos del mapa---------------------------------------------------------
        ...
        var iconoPa = new H.map.Icon('img/iconopa.png')
...
//--------------------------------------------------------PUNTOS--------------------------------------------------------------------------          
        ...
        //Patrimonios
        function marcadores2(map){

          var grupopa = new H.map.Group(); 

          map.addObject(grupopa);  

          grupopa.addEventListener('tap', function (evt) {
            // event target is the marker itself, group is a parent event target
            console.log(evt.target.getData())
            // for all objects that it contains
            var bubble = new H.ui.InfoBubble(evt.target.getGeometry(), {
          ...
          //Parques Nacionales
            var db = firebase.firestore();
            var perRef = db.collection("Patrimonios Culturales");
            perRef.get()
              .then(function(querySnapshot) {
                querySnapshot.forEach(function(doc) {
                console.log("Enlace:" + doc.data().Link);
        ...
        function agregarpa(grupopa, coordinate, html){
          var marcadorPa = new H.map.Marker(coordinate, {icon: iconoPa}); 
          grupopa.addObject(marcadorPa); 
          marcadorPa.setData(html);
        }  
        marcadores2(map);
...
function abrirpopupa(id){
  ...
  var db = firebase.firestore();
  var docRef = db.collection("Patrimonios Culturales").doc(id);    
...
function fnAr(){
  $$('#regs').removeClass('seve').addClass('vr');
}


function fnAl(){
  $$('#logs').removeClass('seve').addClass('vr');
}


function fnAu(){
  $$('#pp').removeClass('vr').addClass('seve');
}



function fnPerfilUsuario(){
  ...
  if (doc.exists) {
      ...
      $$('#nomU').html("Nombre:" + "   " + doc.data().Nombre)
      $$('#apellidoU').html("Apellido:" + "   " + doc.data().Apellido)
      $$('#emailU').html("Email:" + "   " + doc.data().Email)
